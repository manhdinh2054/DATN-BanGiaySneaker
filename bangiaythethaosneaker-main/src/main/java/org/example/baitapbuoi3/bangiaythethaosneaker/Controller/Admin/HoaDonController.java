...
public class HoaDonController {
    ...
    @GetMapping("/admin/quanlihoadon/quanli/search")
    public String searchHoaDonQuanLi(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size,
...
//                  false:
    ...
    @GetMapping("/admin/quanlihoadon/quanli/detail/{id}")
    public String showAndEditDetailHoaDon(@PathVariable("id") int id, Model model) {
        List<DonHangDTO> hdct = dhstorsp.getAllDonHangChiTietByIdHoaDon(id);
        model.addAttribute("hoadonchitietlst", hdct);
        return "admin/quanlihoadonchitiet";
    }


    @GetMapping("/admin/quanlihoadon/lichsu/search")
    public String searchHoaDonLichSu(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size,
                                     @RequestParam(value = "sdtKhachHang", required = false) String sdt,
                                     @RequestParam(value = "trangThai", required = false) int trangThai,
                                     Model model) {
        if (trangThai == 00) {
    ...
    @GetMapping("/admin/quanlihoadon/lichsu/detail/{id}")
    public String showDetailHoaDon(@PathVariable("id") int id, Model model) {
        List<HoaDonDTO> lst = hddtoi.getAllHoaDonChiTietByIdHoaDon(id);
        model.addAttribute("hoadondetail", lst);
        return "admin/lichsuhoadonchitiet";
    }
}
